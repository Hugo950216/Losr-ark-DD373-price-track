    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                values = [float(element.inner_text().split('=')[1].strip().split('金')[0]) for element in elements[:5]]
...
def check_queue(result_queue):
    try:
        prices = result_queue.get_nowait()
        if prices and not any(isinstance(price, str) and price.startswith("Error") for price in prices):
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M")
            update_time_var.set(f"更新時間: {current_time}")
            for i, value in enumerate(prices):
                price_vars[i].set(f"{value:.2f}")
            ...
            error_message = prices[0] if prices else "無法獲取價格"
...
for i in range(5):
    ...
    ttk.Label(price_frame, textvariable=price_vars[i], font=('Roboto', 11, 'bold'), foreground='#2196F3').grid(row=i, column=1, sticky='w', pady=5)
    ttk.Label(price_frame, text="金/人民幣", font=('Roboto', 11)).grid(row=i, column=2, sticky='w', padx=(5, 0), pady=5)
